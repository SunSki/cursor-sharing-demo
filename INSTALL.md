# DotSync / LiveCursors — インストール手順書

既存のWebサイトにリアルタイムドット共有（カーソル位置マーカー）を追加する手順です。  
用途に応じて **2つの方法** があります。
...
```bash
npm install \
  /path/to/dotsync/dist/livecursors-server-0.1.0.tgz \
  /path/to/dotsync/dist/livecursors-client-0.1.0.tgz
```
...
{
  "dependencies": {
    "@livecursors/server": "file:../dotsync/dist/livecursors-server-0.1.0.tgz",
    "@livecursors/client": "file:../dotsync/dist/livecursors-client-0.1.0.tgz"
  }
}
```
